...
#yesterday=$(date -u --date='00:00:00 today' +%s)

OUTPUT_DIR="$(condor_ce_config_val APEL_OUTPUT_DIR)"
OUTPUT_FILE="$OUTPUT_DIR/batch-$(date -u --date='today' +%Y%m%d )-$(hostname -s)"

[[ -d $OUTPUT_DIR && -w $OUTPUT_DIR ]] || fail "Cannot write to $OUTPUT_DIR"

# Build the filter for the history command
CONSTR="EnteredCurrentStatus >= $today && RemoteWallclockTime =!= 0"
